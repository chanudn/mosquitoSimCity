...
# General parameters
# Most of these correspond to more specific definitions found in the lookup_vars() function
# (e.g., the $loc value here corresponds to specific lat/lon values defined in lookup_vars()).
my $dataSrc    = "MERRA_IMERG";      # Climate data source ("GEOS_S2S" or "MERRA_IMERG")
my $loc        = "NuwaraEliya";      # Geographic location/city name
my $tspan_strt = "2017060100";       # Start time for model run (YYYYMMDDHH)
my $tspan_end  = "2017063023";       # End time for model run (YYYYMMDDHH)
...
my $cflag  = "T";      # Whether cloud fraction data is input (T/F)        (set to "T" when using MERRA/IMERG, "F" when using GEOS S2S)
my $sflag  = "T";      # Whether soil temperature data is input (T/F)      (set to "T" when using MERRA/IMERG, "F" when using GEOS S2S)
...
# Filepaths
my $path_exe     = "/home/local/WIN/cyasana1/Code/project_whatchem/whatchem-2.1_CNY/container_modeling/src"; # Path to container.exe file
my $dir_input    = "/mnt/redwood/local_drive/chanud/whatchem_2.1_CNY/input_data";                            # Directory of input data
my $dir_out_data = "/mnt/redwood/local_drive/chanud/whatchem_2.1_CNY/output_data";                           # Directory for output data
# TEST! Change back later.
#my $dir_out_data = "/mnt/redwood/local_drive/chanud/whatchem_2.1_CNY/output_data/TEST";     # TEST Directory for output data
...
#			  $tStep_model, $tStep_input, $missVal, $path_exe, $dir_input, $dir_out_data);
...
for my $loc (@locs){
	for my $year (@years) {
		for my $month (@months) {
			...
			print "Location: ${loc}  Year: ${year}  Month: ${month}\n";

			# first we need to generate the timespan strings based on the year and month of interest
			...
			if (($month == 1) or ($month == 3) or ($month == 5) or ($month == 7) or
				($month == 8) or ($month == 10) or ($month == 12)) { # months with 31 days
				$daysInMon = 31;
			} elsif (($month == 4) or ($month == 6) or ($month == 9) or ($month == 11)) { # months with 30 days
				$daysInMon = 30;
			...
			my $tspan_strt = $year*1000000 + $month*10000 + 100;
			my $tspan_end  = $year*1000000 + $month*10000 + $daysInMon*100 + 23;
			print "Start time: ${tspan_strt}  End time: ${tspan_end}\n";

			my $tspan = "${tspan_strt}_${tspan_end}";

			# next we run the WHATCH'EM code
			whatchem_main($dataSrc, $loc, $tspan, $name_cont, $name_refl, $name_shade, 
						 $wh_mul, $doClim, $cflag, $sflag, $mfill, $fill_intv, $mempty, $empty_intv,
						 $tStep_model, $tStep_input, $missVal, $path_exe, $dir_input, $dir_out_data);
...
# Run model for climatology
my $doClim = "T";
my @locs   = qw/ Negombo Jaffna NuwaraEliya /;
my @months = (1..12);
for my $loc (@locs) {
	for my $month (@months) {
		print "Climatology for 2001-2020.  Location: ${loc}  Month: ${month}\n";
		my $tspan = sprintf("2001_2020_%02d", $month); # zero-pads $month if needed so that it's two digits
		whatchem_main($dataSrc, $loc, $tspan, $name_cont, $name_refl, $name_shade, 
						$wh_mul, $doClim, $cflag, $sflag, $mfill, $fill_intv, $mempty, $empty_intv,
						$tStep_model, $tStep_input, $missVal, $path_exe, $dir_input, $dir_out_data);
	}
...
#    tspan_strt - Start time of input data and of model run (YYYYMMDDHH)
#     tspan_end - End time of input data and of model run (YYYYMMDDHH)
...
#  dir_out_data - Directory for output data
#
sub whatchem_main {
	my ($dataSrc, $loc, $tspan, $name_cont, $name_refl, $name_shade, 
	    $wh_mul, $doClim, $cflag, $sflag, $mfill, $fill_intv, $mempty, $empty_intv,
		$tStep_model, $tStep_input, $missVal, $path_exe, $dir_input, $dir_out_data) = @_;
	...
	# Run names
	my $name_srcLocTime = "$dataSrc-$loc-$tspan"; # name indicating data source, place, and time of input data
	...
	my $path_out_data = "$dir_out_data/$name_srcLocTime";
