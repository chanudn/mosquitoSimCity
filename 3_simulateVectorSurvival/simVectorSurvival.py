...
def calc_monRecord_dev_surv(dataSrc, loc, year, month, intv):
...
#    month       - month of WHATCH'EM data (0 to 12)
#    intv        - intv00, intvF0, intv0E, or intvFE (0 is no intv, F is fill, E is empty)
def save_monRecord_dev_surv(dataSrc, loc, year, month, intv):

    df_dev_hrly, df_dev_surv_dly = calc_monRecord_dev_surv(dataSrc, loc, year, month, intv)
    _, subDir, fileStr_allSpecs = WHATCHEM_specs_to_fpath(dataSrc, loc, year, month, intv)
    
    # Save hourly df
    with open(DIR_WHATCHEM_OUTPUT_DATA + subDir + "df_dev_hrly_" + fileStr_allSpecs + ".txt", 'w') as f:
    ...
    # Save daily  df
    with open(DIR_WHATCHEM_OUTPUT_DATA + subDir + "df_dev_surv_dly_" + fileStr_allSpecs + ".txt", 'w') as f:
        # Convert metadata to a string format suitable for a header
        metadata_str = ''
...
def calc_monRecord_vectorPop(dataSrc, loc, year, month, intv, initDay, initEggs, initLarvae, initPupae):

    _, df_dev_surv_dly = calc_monRecord_dev_surv(dataSrc, loc, year, month, intv)
...
# How do eggs become larvae?
# Magori et al.: see Fig 2 (it neatly summarizes the Focks et al. algorithm described below)
# Focks et al.: (embryonation) complete on day when CD_i > 0.95 (eggs must be embryonated to be able to hatch)
#               (hatch) spontaneous, flood-induced, and temp-dependent
#                         min temp is 22 deg C (let's use mean daily temperature for this, as justified in notes from Dec 3, 2023)
#                         19.7% of newly embryonated eggs hatch spontaneously without submergence
#                         of remaining eggs, 59.6% hatch each day of submergence
#                         eggs submerged on day when embyronation is completed hatch immediately

# How do larvae become pupae?
# Focks et al.: (pupation) on day when CD_i > 0.95 and minimum weight achieved (we'll ignore min weight)
#                          5% die during pupation (only 95% survive) as a proxy for deaths during larval development due to genetic abnormalities (Gilpin & McClelland 1979)

# How do pupae become adults?
# Focks et al.: (emergence) on day when CD_i > 0.95
#                           17% die during eclosion (only 83% survive)
# Skeeter Buster: [same as Focks et al.] 17% die during eclosion (only 83% survive)
...
#    initLarvae  - initial population of larvae
#    initPupae   - initial population of pupae
def save_monRecord_vectorPop(dataSrc, loc, year, month, intv, initDay, initEggs, initLarvae, initPupae):

    df_vectorPop_dly = calc_monRecord_vectorPop(dataSrc, loc, year, month, intv, initDay, initEggs, initLarvae, initPupae)
    _, subDir, fileStr_allSpecs = WHATCHEM_specs_to_fpath(dataSrc, loc, year, month, intv)
    ...
    # Save df
    with open(DIR_WHATCHEM_OUTPUT_DATA + subDir + "df_vectorPop_dly_" + fileStr_allSpecs + '-' + popStr + ".txt", 'w') as f:
        # Convert metadata to a string format suitable for a header
        metadata_str = ''
...
# What to plot!
# We have a lot of daily variables...
# Climate inputs: PRCP (mm), TA (C), VPD (kPa)
# WHATCH'EM outputs: WH (mm), TW (C), TW_min (C), TW_max (C)
# Dev/surv outputs: (for each of eggs, larvae, pupae) dev, surv, surv_temp, surv_desicc
# 
# Also we have hourly variables...
# Climate inputs: PRCP (mm), TA (C), VPD (kPa)
# WHATCH'EM outputs: WH (mm), TW (C)
# Dev outputs: (for each of eggs, larvae, pupae) dev
#
# And we have all these variables for each of our three locations!
#
# For each location...
#
# First, we want to see how well the climate inputs and WHATCH'EM outputs correlate.
# If they correlate too closely, then it's questionable how much added value there is
# in using WHATCH'EM output as predictors of dengue (since using the climate inputs as
# predictors would be just as effective, or we could just accurately calculate the
# WHATCH'EM output based on climate inputs without actually running the model).
#   WH (mm) vs PRCP (mm)  [hourly]
#   TW (C) vs TA (C)        [hourly]
#
# We can also see how much of an impact the time resolution has: how much does the correlation
# change with daily data? (When looking at daily data we can also look at min/max daily temps,
# which impact the survival factor.)
#   WH (mm) vs PRCP (mm)   [daily]
#   TW (C) vs TA (C)         [daily]
#   TW_min (C) vs TA_min (C) [daily]
#   TW_max (C) vs TA_max (C) [daily]
#
# Next, we want to see how mosquito development rate relates to water temperature (the only
# variable that development rate depends on). Since we calculated dev rate using hourly data, 
# if we plot hourly dev rate vs hourly water temp, all points should just map out the
# dev rate equation (i.e., all points should fall along a single curve).
#   dev_* vs TW (C) [hourly]   (for each of eggs, larvae, pupae) 
#
# What is the impact of time resolution? Given that the dev rate equation is non-linear,
# would we get dramatically different results if we calculated dev rate with daily data?
# To see the difference, let's look at daily dev rate vs mean daily temp. The impact of
# the dev rate equation's non-linearity is the extent to which the daily data doesn't 
# fall on a single curve.
#   dev_* vs TW (C) [daily]   (for each of eggs, larvae, pupae) 
#
# Similarly, let's see how mosquito survival factor relates to other variables. Unlike dev rate,
# which we originally calculated as an hourly variable, survival factor is originally calculated 
# as a daily variable. Therefore we don't need to consider any hourly data here.
# We'll separately consider the temperature-dependent and desiccation-dependent survival factors.
# Temperature-dependent survival factor depends on min/max water temp.
#   surv_temp_* vs TW_min (C) [daily]   (for each of eggs, larvae, pupae)
#   surv_temp_* vs TW_max (C) [daily]   (for each of eggs, larvae, pupae)
# Desiccation-dependent survival factor depends on water height and vapor pressure deficit.
# (It also depends on sun exposure, but we don't need to consider that here since all our 
# experiments hold that fixed (half-shaded)).
#   surv_desicc_* vs WH (mm)   [daily]   (for each of eggs, larvae, pupae)
#   surv_desicc_* vs VPD (kPa) [daily]   (for each of eggs, larvae, pupae)
#
# That wraps up considering how variables correlate to one another!
#
# Next up: considering interannual variability.
# For this we plot a given variable vs. time for each year+month. On a given plot we will plot
# (a) the timeseries of the variable for each year, and 
# (b) the climatological mean timeseries of that variable,
# which will show us how much the individual years of data differ from the climatological mean.
# Since we've previously looked at variability in the the climate inputs (precip, temp, humidity) 
# and in the WHATCH'EM outputs (water height, water temp), here we'll just look at the dev rate
# and survival factor.
# (note: we never look at daily min/max air temp or daily min-max water temp, which might be
# good to look at at some point).
#
# First, we look at interannual variability in the development rate. The hourly timeseries will
# look messy because of the diurnal cycle, so we'll look at the daily timeseries.
#   dev_* vs time (i.e., days of month) [daily]   (for each of eggs, larvae, pupae; for each month)
#
# Speaking of the diurnal cycle in development rate, it'd be interesting to see this!
# Let's also plot this for fun.
#   dev_* vs time (i.e., hours of day) [hourly]   (for each of eggs, larvae, pupae; for each month)
#
# Next, we look at interannual variability in the survival factor. As before, we'll separately
# consider the temperature-dependent and desiccation-dependent survival factors.
#   surv_temp_* vs time (i.e., days of month)   [daily]   (for each of eggs, larvae, pupae; for each month)
#   surv_desicc_* vs time (i.e., days of month) [daily]   (for each of eggs, larvae, pupae; for each month)
#
# We'll also look at the overall survival factor.
#   surv_* vs time (i.e., days of month)   [daily]   (for each of eggs, larvae, pupae; for each month)
#
# I think that's everything!
# So for a given location we're going to have...
#   - 24 plots looking at variable correlations
#   - 180 plots looking at interannual variability
# So across our three locations we'll have 612 plots. Damn.
# 
# In terms of code:
# We need to write a function for plotting one variable against another (for looking at variable correlations).
# We already have some code from plottingFns_WHATCHEM_analysis.py that we could probably repurpose for looking
# at interannual variability.
