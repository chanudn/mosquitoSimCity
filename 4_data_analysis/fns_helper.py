...
# ************************************************
import warnings
warnings.simplefilter(action='ignore', category=FutureWarning) # This hides the warning that's thrown due to read_WHATCHEM_input()
                                                               # calling pd.read_table() with the args parse_dates and date_parser. 
                                                               # Python suggests using pd.to_datetime instead.
                                                               # However I tried this and couldn't get it to format the dates in the
                                                               # same way, so idk we're just going to hide the warnings for now.
import pandas as pd
import numpy as np
...
def get_tbounds_yyyymmddhh(year, month):
    ...
    tStrt = year*1000000 + month*10000 + 100
    tEnd  = year*1000000 + month*10000 + daysInMon*100 + 23
...
def WHATCHEM_specs_to_fpath(dataSrc, loc, year, month, intv):
    ...
    subDir  = dataSrc + '-' + loc + "-" + tspan + "/"
    dir_in  = DIR_WHATCHEM_OUTPUT_DATA + subDir
    ...
    filepath_in  = dir_in + "container_output_" + fileStr_allSpecs + ".txt"

    return filepath_in, subDir, fileStr_allSpecs
...
def convert_df_vectorPop_dly2mon(df_dly):
    ...
    df_mon = df_dly.resample('M').agg(agg_fns)
    df_mon.index = df_mon.index.to_period('M').to_timestamp() # default time index is for end of month, so shift it to beginning of month
    return df_mon
